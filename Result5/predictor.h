...
#define MAX_ASSOC	256
	...
	// hash global history and local history
	unsigned int hash_ghistlocal(int n) {
	
		unsigned int *lo = &local_histories[hashlocal()%nlocal_histories];
		unsigned int hash,mask,temp=0;
		mask = ((1<<local_history_length) - 1);
		for(int i=0;i<n;i++)
		temp^=ghist_words[i]&mask;
		*lo&=mask;
		hash = (*lo)^temp;
		return hash;
	}
